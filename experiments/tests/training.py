...
# load configs
with open("config/environment.yaml") as env_yaml:
...
with open('config/model.yaml') as model_yaml:
...
for i in range(1, env_config['repetitions'] +1):
    ...
    msi_dict = load_annotated_msis(**data_config['recordings_and_annotations']) 
    ...
    eval_model.load_weights(report_dir+"models/"+model_config['name']+"_"+data_config['name'])
    ...
    print_and_plot_confusion_matrix(val_gts, val_preds, label_names, savedir=report_dir, suffix="val")
    print_and_plot_confusion_matrix(train_gts, train_preds, label_names, savedir=report_dir, suffix="train")
